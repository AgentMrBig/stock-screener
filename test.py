...
def setTime(time):
    time_obj = datetime.strptime(time, '%b %d, %Y')     # Convert strings to datetime.datetime
    time_stamp = datetime.timestamp(time_obj)   # Convert datetime.datetime to timestamp
    time_stamp = int(time_stamp)                # next I remove the dec by converting from float to int
    return time_stamp
...
df = getFinanceData(setTime('Dec 25, 2010'), setTime('Oct 22, 2020'), US30, 'D')
